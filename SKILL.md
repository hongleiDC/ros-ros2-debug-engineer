...
description: Develop, review, migrate, and debug ROS 1 and ROS 2 repositories, packages, nodes, launch files, interfaces, TF trees, calibration, timing, rosbag data, QoS, DDS discovery, lifecycle nodes, executors, build failures, runtime failures, and LiDAR-IMU-RTK pipelines. Use when work requires evidence-based ROS diagnosis or code changes, especially long-running debugging where the original engineering goal, success criteria, experiments, and decisions must remain explicit. Before project-specific conclusions, build a project fact model and a goal contract. Default to read-only diagnosis; modify, persist, publish, or activate hardware only with explicit authorization.
---
